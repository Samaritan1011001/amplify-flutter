...
class EventClient {
  ...
  static final AmplifyLogger _logger =
      AmplifyLogger.category(Category.notifications).createChild('EventClient');
  ...
  Future<void> recordEvent(Event event) async {
    ...
    try {
      ...
      _logger.info("Event request-> ${EventsRequest(batchItem: batchItems)}");

      _logger.info("Response to record event -> $endpointResponse");
      ...
      _logger.info("successfully recorded event");
